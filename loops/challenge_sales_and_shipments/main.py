...
# index iteration through the products list subtract the units_sold from the current inventory
for i in range(len(products)):
    products[i][1] = products[i][1]-units_sold[i][1]
# index interation through the products list add the shipment_received to the update inventory
for i in range(len(products)):
    products[i][1] = products[i][1]+shipment_received[i][1]
# final stock levels for all products
